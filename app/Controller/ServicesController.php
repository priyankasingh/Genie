...
class ServicesController extends AppController {
	...
	public $components = array('RequestHandler','Twitter.Twitter');
	
	function beforeFilter(){
		$this->Auth->allow( array( 'index', 'view', 'my-map' ) );
		parent::beforeFilter();
	}
	...
	public function twitter(){
		...
		debug($this->Twitter->userTimeline("ohdigital"));
		die();
	}
...
/*		if($this->RequestHandler->isAjax()){
	...
	public function index($selected_parent_slug = null, $selected_category_slug = null, $selected_service_slug = null){
		// pr("Parent <br>" . $selected_parent_slug);
		// pr("Category <br>" . $selected_category_slug);
		// pr("Service <br>" . $selected_service_slug);

		// Get network members
		// Has response?
		$this->loadModel('Response');
		$response_id = $this->Session->read( 'response' );
		$network_members = $response_id ? $this->Response->find('first', array(
		...
		// pr($network_members);
		$this->set(compact('network_members'));
	...
	public function admin_edit($id = null) {
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			if ($this->Service->saveAll($this->request->data)) {
				...
				$this->Service->Video->deleteAll( array(
					'NOT' => array(
						'Video.id' => $videoIds,
					),
